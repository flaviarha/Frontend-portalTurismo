...
const LoginForm = () => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post("https://backend-portalturismo-3rae.onrender.com/api/auth/login", {
        email,
        password
      })
  ...
  return (
    <div className="bg-white p-8 rounded-2xl shadow-lg w-full max-w-md">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <button
          type="submit"
          className="bg-red-700 hover:bg-red-800 text-white px-6 py-2 rounded-lg transition duration-300 w-full sm:w-full mt-2"
        >
          Entrar
        </button>
      </form>
